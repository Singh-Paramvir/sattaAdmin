...
const Gajiyabad = () => {
  ...
  const fetchGajiyabadData = async () => {
    try {
  ...
  const handleSubmit = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.post("/api/setResult1", { token: token, value: dropdownValue });
  ...
  const handleViewDetailsClick = async (item) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post("/api/getDetails", { slotId: 2, token: token });
        ...
        <div className="container">
          <div className="row">
            ...
            <div className="col-md-12">
              ...
              >
                <h2 className="dashboard-heading">Gajiyabad Data</h2>
              </div>
